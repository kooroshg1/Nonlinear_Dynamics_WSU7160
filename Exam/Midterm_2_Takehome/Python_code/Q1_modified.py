...
omega0, alpha, beta = sym.symbols('omega0 alpha beta', real=True)
...
equ = ddt2 * x + omega0**2 * x + eps * alpha * x**2 + eps * beta * x**3
equ = equ.expand()
...
# # Equation for eps0
X0 = A(T1) * sym.exp(sym.I * omega0 * T0) + sym.conjugate(A(T1) * sym.exp(sym.I * omega0 * T0))
